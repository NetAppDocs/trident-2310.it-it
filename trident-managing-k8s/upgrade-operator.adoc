...
NOTE: Per eseguire l'aggiornamento da Astra Trident installato utilizzando l'operatore namespace-scoped (versioni dalla 20.07 alla 20.10), usa le istruzioni di aggiornamento per la versione di Trident che hai installato.
...
.Fasi
. Se link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["Installato Astra Trident utilizzando Helm"], puoi usare  `helm upgrade trident netapp-trident/trident-operator --version 23.10.0` per aggiornare in un unico passaggio. Se non hai aggiunto il Helm repo o non puoi usarlo per aggiornare:
+
...
NOTE: Prima di passare da un metodo di installazione all'altro, dai un'occhiata a link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["Passaggio da un metodo di installazione all'altro"]
