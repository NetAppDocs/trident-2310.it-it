...
Sì, Astra Trident può essere installato offline. Vedi link:./trident-install/kubernetes-deploy.html["qui"].
...
Il programma di installazione di Trident crea un Kubernetes ClusterRole, che ha accesso specifico alle risorse PersistentVolume, PersistentVolumeClaim, StorageClass e Secret del cluster Kubernetes. Vedi link:./trident-install/kubernetes-customize-deploy-tridentctl.html["qui"].
...
Puoi generare e modificare localmente i file manifest esatti che Astra Trident utilizza per l'installazione, se necessario. Vedi link:./trident-install/kubernetes-customize-deploy-tridentctl.html["qui"].
